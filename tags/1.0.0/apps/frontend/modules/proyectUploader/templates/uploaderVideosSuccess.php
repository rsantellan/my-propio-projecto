<?php
  ...
  include_component('uploader', 'uploader', array(
    'upload_url'        => url_for('uploader/uploadVideos?'. ini_get('session.name') . '=' . session_id() . '&upload=mastodonte'),     //ruta al action que procesa la imagen y la sube
    'file_types'        => sfConfig::get( 'sf_plugins_upload_content_type_' . $objectClass, '*.mpg;*.MPG' ),  //formatos soportados
