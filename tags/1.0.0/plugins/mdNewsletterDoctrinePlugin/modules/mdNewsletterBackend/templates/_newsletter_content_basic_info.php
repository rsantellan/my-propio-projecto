<div id="md_basic_<?php echo $form->getObject()->getId() ?>" class="md_open_object_top">
    ...
    <div class="md_blocks">
        <h2><?php echo __("newsletter_cuerpo");?></h2>
        <div style="float: left; padding: 2px; margin: 2px;" class="<?php if($form['body']->hasError()):?>error_msg<?php endif; ?>">
            <?php echo $form['body']->render(); ?>
        </div>
        <div><?php if($form['body']->hasError()): echo __("newsletter_cuerpo") .': '. $form['body']->getError();  endif; ?></div>
    </div>
   <div class="clear"></div>
</div>
<!--FIN ABIERTO TOP-->
