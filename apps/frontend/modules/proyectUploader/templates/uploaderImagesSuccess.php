...
var_dump($sf_user->isAuthenticated());
  ...
  include_component('uploader', 'uploader', array(
    'upload_url'        => url_for('proyectUploader/uploadImages?'. ini_get('session.name') . '=' . session_id() . '&upload=mastodonte' . (isset($planos)?'&p=true':'')),     //ruta al action que procesa la imagen y la sube
    'file_types'        => sfConfig::get( 'sf_plugins_upload_content_type_' . $objectClass, '*.jpg;*.jpeg;*.gif;*.png' ),  //formatos soportados
